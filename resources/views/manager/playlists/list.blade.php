...
<table border="0">
    @foreach($playlists as $key => $playlist)
        <tr class="p-2">
            <td><a href="{{ route('playlists.show', $playlist->id) }}">{{ $playlist->name }}</a></td>
            <td><a href="{{ route('playlist.destroyAll', $playlist->id) }}" onclick="return confirm('ban chac chan xoa?')">Xoa</a></td>
        </tr>
    @endforeach
</table>
